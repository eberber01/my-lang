...
struct AstNode* parse_expression(struct Token* tokens, int token_len, int* current_token){
    ...
    struct AstNode* left= parse_term(tokens, token_len, current_token);
    while(tokens[*current_token].type == ADD || tokens[*current_token].type == SUB){
        char* value = tokens[*current_token].value;
        (*current_token)++;
        struct AstNode* right = parse_term( tokens,token_len,current_token);
...
struct AstNode* parse_term( struct Token* tokens, int token_len, int* current_token){
    ...
    struct AstNode* left = parse_factor(tokens, token_len, current_token);
    while(tokens[*current_token].type == DIV || tokens[*current_token].type == MULT){
        char* value = tokens[*current_token].value;
        (*current_token)++;
        struct AstNode* right = parse_factor(tokens,token_len,current_token);
        left = make_ast_node(BINARY_EXPR,value , left, right);
    }
    return left;
...
struct AstNode* parse_factor(struct Token* tokens, int token_len, int* current_token){
    if(tokens[*current_token].type == NUM){
        ...
        (*current_token)++;
        struct AstNode* ret =  parse_expression( tokens, token_len, current_token);
